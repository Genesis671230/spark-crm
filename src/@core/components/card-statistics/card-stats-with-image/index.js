...
const CardStatsCharacter = ({data}) => {
  // ** Vars
  const {title1, title2, stats1, status2, src, trend, trendNumber} = data
        ...
        <Grid container>
          <Grid item xs={6}>
            <Box sx={{height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between'}}>
              <Box>
                <Typography sx={{mb: 1.5, fontWeight: 600, whiteSpace: 'nowrap'}}>{title1}</Typography>
              ...
              <Box>
                <Typography sx={{mb: 1.5, fontWeight: 600, whiteSpace: 'nowrap'}}>{title2}</Typography>
                <Box sx={{display: 'flex', flexWrap: 'wrap', alignItems: 'center'}}>
                  <Typography variant='h5' sx={{mr: 1.5}}>
                    {status2}
                  </Typography>
          ...
          <Grid item xs={6} sx={{display: 'flex', alignItems: 'flex-end', justifyContent: 'flex-end'}}>
            <img src={src} alt={title1} height={134}/>
          </Grid>
        </Grid>
      </CardContent>
    </Card>
